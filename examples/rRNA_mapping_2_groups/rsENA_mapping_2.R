...
hmean=rowMeans(re.data[,Hsamples])  #Hsamples列名
...
re.data=data.frame(length=rownames(re.data),hmean=hmean,lmean=lmean)
...
dp$variable=factor(dp$variable,levels=c("hmean","lmean")) # "nmean",
...
p=ggplot(dp, aes(x = length, y = value, color=variable,group=variable))+
  ...
  ylab("Coverage(RPM)")+xlab("Length of nucleotides")+
  labs(color="group")+labs(fill="group")+
  scale_x_discrete(breaks = seq(min(dp$length), max(dp$length), by = 10))
p
# ggsave("RNA5_CGGGGCGCGGGACATGTGG.pdf",p,width=8,height=4)
# ggsave("RNA5_2_CGGGGCGCGGGACATGTGG.pdf",p,width=8,height=4)
